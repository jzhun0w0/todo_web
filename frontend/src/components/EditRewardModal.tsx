...
interface Props {
  reward: RewardItem
  onClose: () => void
  onSubmit: (id: number, costRp: number, costCuti: number) => void
}

export default function EditRewardModal({ reward, onClose, onSubmit }: Props) {
  const [costRp, setCostRp] = useState(reward.cost_rp.toString())
  const [costCuti, setCostCuti] = useState(reward.cost_cuti.toString())

  const parsedRp = parseInt(costRp, 10) || 0
  const parsedCuti = parseInt(costCuti, 10) || 0
  const isValid = parsedRp > 0 || parsedCuti > 0

  const handleSubmit = () => {
    if (!isValid) return
    onSubmit(reward.id, parsedRp, parsedCuti)
    onClose()
  }
    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal" onClick={e => e.stopPropagation()}>
        ...
        <div className="form-group">
          ...
          <div style={{ padding: '10px 14px', background: 'var(--bg-hover)', borderRadius: 'var(--radius-sm)', fontSize: '0.95rem', color: 'var(--text-primary)', fontWeight: 500 }}>
            {reward.name}
          </div>
        </div>
